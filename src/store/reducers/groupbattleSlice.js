...
export const groupbattleSlice = createSlice({
    ...
    reducers: {
        ...
        setQuizApiData: (groupbattle, action) => {
            groupbattle.quizApiData = action.payload.data
        }
    }
});

export const { groupbattleSuccess,groupbattleDataClearSuccess,groupbattleShowScore,setQuizApiData } = groupbattleSlice.actions;
...
export const setDataForSetQuizApi = (data) => {
    store.dispatch(setQuizApiData({data}))
}
// selector
...
export const GetSetQuizApiData = createSelector(
    state => state.GroupBattle,
    GroupBattle => GroupBattle.quizApiData,
)