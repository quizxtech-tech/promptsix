...
const FirebaseData = () => {
  // Skip Firebase initialization during SSR to prevent crashes
  if (typeof window === 'undefined') {
    return {
      auth: null,
      db: null,
      fetchToken: async () => { },
      onMessageListener: async () => null,
      signOut: () => { },
      firebaseApp: null,
    };
  }
  ...
  // Also skip if web settings haven't loaded yet (config values undefined)
  if (!websettingsdata?.firebase_api_key) {
    return {
      auth: null,
      db: null,
      fetchToken: async () => { },
