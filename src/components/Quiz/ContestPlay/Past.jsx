          ...
          <>
            {data ? (
              data?.map((pastData, index) => {
                ...
                const titleTruncated = pastData?.name && pastData?.name.length > 23;
                const descTruncated = pastData?.description && pastData?.description.length > 27;
                    ...
                    <div className="rounded-[15px] p-[10px_14px] bg-[var(--background-2)] border-2 border-white/10 darkSecondaryColor">
                      <div>
                        <img
                          ...
                          src={getImageSource(pastData?.image)}
                          alt="wrteam"
                        />
                      </div>
                      <div>
                        <div className="p-2">
                          <div className="relative flex items-center justify-between text-left text-[20px] font-bold text-text-color">
                            <div>
                              <h3>{isExpanded ? pastData?.name : truncate(pastData?.name, 23)}</h3>
                              <p className="text-left text-text-color dark:text-[#FFFFFF99] text-[16px] my-1">{isExpanded ? pastData?.description : truncate(pastData?.description, 27)}</p>
                            </div>
                            {showButton && (
                              <button
                        ...
                        <div className="p-[6px_12px]">
                          ...
                          <div
                            ...
                            onClick={() => LeaderBoard(pastData?.id)}
                          >
