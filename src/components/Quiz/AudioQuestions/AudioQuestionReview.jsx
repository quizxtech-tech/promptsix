...
import { decryptAnswer, getImageSource, handleBookmarkClick, imgError, reportQuestion } from '@/utils'
...
function AudioReviewAnswer({ questions, goBack, reportquestions }) {
  ...
  return (
    <React.Fragment>
      <div className="morphisam flex-center !justify-between max-991:flex-wrap gap-1  max-991:gap-5 max-479:!justify-center darkSecondaryColor">
        <div className="text-start mt-2">
          <h4 className="font-extrabold text-[32px] max-991:text-[22px]">
            {t("review_answers")}
          </h4>
        </div>
        <div className="flex-center gap-7">
          ...
          {reportquestions ? (
            <div className="bg-[#09002921] text-center rounded-full h-[56px] w-[56px] flex-center text-base">
              <button
      ...
      <div className="morphisam mb-6 lg:flex darkSecondaryColor">
        <div className="w-full lg:w-2/3 flex-center h-auto bg-white rounded-[10px] mb-[10px] flex-col py-4 text-center dark:bg-[#3c3555]">
          <p className="px-4 mb-2">{questions[currentQuestion]?.question}</p>
          ...
          <div className="audio_player flex justify-center w-full text-center py-3">
            <audio
      ...
      <div className="flex justify-evenly mb-14 mt-6">
        <div className="">
          <button
            className="reviewBtn"
      ...
      <div className="text-center">
        <div className="flex flex-start mt-5 my-2 text-bold">
          {questions[currentQuestion]?.note ? (
            <p className="text-lg">{t("note") + " : " + questions[currentQuestion]?.note}</p>
          ) : (
            ""
          )}
        </div>
